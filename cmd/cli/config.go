...
func addConfigGenerateCommand(rootCmd *cobra.Command) {
	configGenCmd := &cobra.Command{
		...
		Long:  "Exports the default configuration to config/config.yml, config/config.json, and config/.env.example files",
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			ymlFile, err := os.Create("config/config.yml")
			if err != nil {
				return fmt.Errorf("failed to create config/config.yml: %w", err)
			}
			defer ymlFile.Close()
			...
			fmt.Println("Exported config to config/config.yml")

			jsonFile, err := os.Create("config/config.json")
			if err != nil {
				return fmt.Errorf("failed to create config/config.json: %w", err)
			}
			...
			fmt.Println("Exported config to config/config.json")
